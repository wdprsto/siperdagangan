...
class Login extends CI_Controller {
    ...
    public function index()
    {
		...
		$params['change_uname_notice'] = $this->session->flashdata('change_uname_notice');
    ...
    public function do_login()
    {
        ...
        else
        {
            ...
            if ($this->login->is_user_exist())
            {
                ...
                {
					//login data yang nantinya digunakan sebagai session_data()
                    $login_data = [
                        'is_login' => TRUE,
                        'user_id' => $this->login->logged_username(),
						'user_role' =>  $this->login->get_role(),
                        'login_at' => time(),
                    ...
                    else
                    {
                        ...
                        $redir_to = ($role == 'admin') ? 'admin' : 'kasir';
                    }
                ...
                {
                    // $this->session->set_flashdata('login_flash', 'Password salah!'.$password.":".$user_password.":".$this->login->get_passwordku().":".$this->login->get_username().":".$this->login->get_role().":".$this->login->get_passworddaridb());
