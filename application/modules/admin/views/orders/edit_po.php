				...
				<div class="row">
					<div class="col-md-6">
						...
						<form role="form" action="#" method="POST" id="addPoForm" enctype="multipart/form-data" >
								<div class="form-group row disabled">
									...
									<div class="col-md-8">
										<input class="form-control" name="nomor_po" type="number" id="nomor_po" required placeholder="Masukkan Nomor PO" value="<?php echo $po_id ?>">
								...
								<div class="form-group disabled row">
									...
									<div class="col-md-8">
										<input class="form-control tanggal_po"  name="tanggal_po"  type="datetime-local" id="tanggal_po" required>
									</div>
								</div>
						
								<div class="form-group pertama-buka " style="display:none">
									<div class="row mx-auto justify-content-center">
										<button type="button" name="permanentPoBtn" class="btn btn-primary my-2 permanentPoBtn" >Simpan Permanen</button>
										<button type="button" name="savePoBtn" class="btn btn-success my-2 savePoBtn" >Simpan Sementara</button>
										<button type="button" name="cancelPoBtn" class="btn btn-danger my-2 cancelPoBtn " >Hapus PO</button>
								...
								<div class="form-group row kedua-buka" style="display:none">
					...
					<div class="col-md-6">
						<div class="card-body">
								...
								<div class="form-group row div_nomor_faktur">
									<label for="nomor_faktur" class="col-md-4 col-form-label form-control-label ">No. Faktur</label>
									<div class="col-md-8">
										<input class="form-control nomor_faktur" name="nomor_faktur" type="text" id="nomor_faktur"  placeholder="Masukkan Nomor Faktur">
									</div>
								</div>
								<div class="form-group row div_tanggal_faktur">
									<label for="tanggal_faktur" class="col-md-4 col-form-label form-control-label">Tanggal Faktur</label>
									<div class="col-md-8">
										<input class="form-control tanggal_faktur" name="tanggal_faktur" type="datetime-local" id="tanggal_faktur" >
									</div>
								</div>
								<div class="form-group row div_jatuh_tempo">
									<label for="jatuh_tempo" class="col-md-4 col-form-label form-control-label">Jatuh Tempo</label>
									<div class="col-md-8">
										<input class="form-control jatuh_tempo" name="jatuh_tempo" type="date" id="jatuh_tempo" >
									</div>
									</form>
								</div>
                  ...
                  <div class="card-body px-lg-5 py-lg-5">
                      <form role="form" action="#" method="POST" id="addProductForm">
                              ...
                              <div class="input-group input-group-merge input-group-alternative input-data-barang">
                                  ...
                                  <input name="jumlah_pesanan" class="form-control add-jumlah jumlah_pesanan" placeholder="Jumlah barang yang dipesan" type="number" min="1"  required>
		...
		<div class="modal-content bg-gradient-danger">
			...
			<div class="modal-body">
				<div class="py-3 text-center">
					...
					<p>Perubahan data pada PO bukan berarti menghapus semua data barang yang ada. Untuk menghapus PO, buka menu PO lalu tekan tombol hapus.</p>
				</div>
			</div>
			<div class="modal-footer">
...
<script>
  ...
  $(document).ready(function() {
		...
		$.ajax({
			method: 'GET',
			url: '<?php echo site_url('admin/orders/api?action=view_data_po'); ?>',
			data: {id_po: id_po},
			success: function(res) {
				if (res.data) {
					var d  = res.data;
					var tanggal_po = new Date(d.po_tgl);
					tanggal_po.setMinutes(tanggal_po.getMinutes() - tanggal_po.getTimezoneOffset());

					$('.tanggal_po').val(tanggal_po.toISOString().slice(0,16));
					$('.supplier').val(d.po_idsup);
					if(d.po_nofak!=''){$('.nomor_faktur').val(d.po_nofak);}
					if(d.po_tglfak!='0000-00-00 00:00:00'){
						var tanggal_faktur = new Date(d.po_tglfak);
						tanggal_faktur.setMinutes(tanggal_faktur.getMinutes() - tanggal_faktur.getTimezoneOffset());
						$('.tanggal_faktur').val(tanggal_faktur.toISOString().slice(0,16));
					}
					$('.jatuh_tempo').val(d.po_tgljatuhtempo);

					if(d.po_status=='permanen'){
						$('.kedua-buka').show();
						$('.div_tanggal_faktur').addClass("disabled");
						$('.div_nomor_faktur').addClass("disabled");
						$('.div_jatuh_tempo').addClass("disabled");
      ...
      "language" : {
		...
		table.on('click', function(){
			...
			setTimeout(() => {
				tdata = table.rows( { selected: true } ).data()[0];
    ...
    $('#addPoForm').submit(function(e) {
      ...
      $('.err').empty();
		...
		$(document).on('click', '.savePoBtn', function() {
				...
				$.ajax({
					...
					success: function (res) {
						if (res.code == 201) {
							...
							setTimeout(function() {
								btn.html('Simpan Sementara');
							}, 1000);
						}
					}
				})
			}else{
				$('#modal-notification-0').modal('show');
			...
			//MENYIMPAN PERMANEN/simpan permanen PO
			$(document).on('click', '.permanentPoBtn', function() {
			var btn = $('.permanentPoBtn');
			var id_po = $('#nomor_po').val();
			var no_fak = $('#nomor_faktur').val();
			var tgl_fak = $('#tanggal_faktur').val();
			var jatuh_tempo =  $('#jatuh_tempo').val();
			...
			if(no_fak!='' && tgl_fak!=''){
				...
				if(total_barang>0){
					$.ajax({
						method: 'POST',
						url: '<?php echo site_url('admin/orders/api?action=permanent_save_po'); ?>',
						data: {
							id_po: id_po,
							no_fak: no_fak,
							tgl_fak: tgl_fak,
						...
						success: function (res) {
							if (res.code == 201) {
								...
								setTimeout(function() {
									btn.html('Simpan Permanen');
									$('.pertama-buka').remove();
									$('.kedua-buka').show();
									$('.addPoInfo').text('Data PO berhasil disimpan permanen.')
								}, 1000);
				...
				$('#modal-notification-faktur').modal('show');
			}
