...
<body>
	...
	<table id="table" style="table-layout: fixed">
		<thead>
			<tr class="align-top">
				<th scope="col" colspan="2" class="text-left"><span>Nama Barang</span></th>
				<th scope="col" class="text-left" >Qty</th>
				<th scope="col">Harga</th>
				<th scope="col" colspan="2" class="text-right">Jumlah</th>
		...
		<tbody>
			...
			<?php else: ?>
				<?php foreach ($barang_nota as $barang) : ?>
					<tr class="align-top">
						<td colspan="2"><?php echo $barang->brg_nama; ?></td>
						<td><?php echo $barang->jual_jumlahbrg . ' ' . $barang->brg_satuan; ?></td>
						<td class="text-right"><?php	echo format_rupiah($barang->brg_hargajual); ?>
								<br>
								<?php if($barang->jual_diskon) echo '<span>Diskon:</span>'; ?>
						</td>
						<td class="text-right" colspan="2"><?php echo format_rupiah($barang->jual_hargajual); ?>
								<br>
								<?php if($barang->jual_diskon) echo '(' . format_rupiah($barang->jual_diskon).')'; ?>
						</td>
						<!-- <td><?php // echo format_rupiah($barang->jual_diskon); ?></td> -->
					</tr>
				<?php endforeach ?>
			<?php endif;?>
		</tbody>
		<tfoot>
			<tr>
				...
				<td colspan="2"><span class='d-block mt-2  text-right'><?php echo 'Rp.' . format_rupiah($total_biaya_nota->total_hargakotor); ?></span></td>
			</tr>
			<tr>
				<td colspan='4'><span class='d-block mb-2'>Total Diskon</span></td>
				<td colspan="2"><span class='d-block mb-2  text-right'><?php echo 'Rp.' . format_rupiah($total_biaya_nota->total_diskon); ?></span></td>
			</tr>
		</tfoot>
		<tr>
			<td colspan='4'><span class='d-block mt-2 font-weight-bold'>Jumlah Tagihan</span></td>
			<td class="font-weight-bold  text-right" colspan="2"><span class='d-block mt-2'><?php echo 'Rp.' . format_rupiah($total_biaya_nota->total_hargabersih); ?></span></td>
		</tr>
		<tr><td colspan='5'><span>&nbsp;</span></td></tr>
		<tr>
			<td colspan='3'><span class=' text-left'>Penerima</span></td>
			<td colspan='3' class=" text-right"><span class=''>Kasir,</span></td>
		</tr>
		<tr><td colspan='5'><span>&nbsp;</span></td></tr>
		<tr>
