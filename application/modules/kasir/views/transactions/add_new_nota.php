						...
						<div class="card-body">
							<form role="form" action="#" method="POST" id="addNotaForm" enctype="multipart/form-data">
								<div class="form-group row disabled">
									...
									<div class="col-md-8">
										<input class="form-control" name="nomor_nota" type="number" id="nomor_nota" required placeholder="Masukkan Nomor PO" value="<?php echo $last_id->nota_id + 1 ?>">
										<input type="hidden" id="simpan_nomor_nota" value="<?php echo $last_id->nota_id + 1 ?>">
										<span class="id-error text-danger text-sm-left"></span>
									</div>
								</div>
								...
								<div class="form-group row pertama-buka">
									<div class="mx-auto">
										<button type="submit" name="submit" class="btn btn-primary my-2 addNotaBtn ">Buat Nota</button>
									</div>
								</div>
								<span class="addNotaInfo text-danger text-sm-left pertama-buka" style="font-size:.875rem">*Pastikan Nomor Nota sudah terisi sebelum membuat dokumen Nota</span>
								<div class="form-group row kedua-buka">
									<div class="mx-auto">
										...
										<button type="button" name="cancelNotaBtn" class="btn btn-danger my-2 cancelNotaBtn  mr-2">Batalkan</button>
										<div class="btn-group">
								...
								<span class="addNotaInfo text-info text-sm-left kedua-buka" style="font-size:.875rem"></br></br></span>
						...
						<form role="form" action="#" method="POST" id="addProductForm">
							...
							<div class="form-group mb-3">
								<div class="input-group input-group-merge input-group-alternative input-data-barang">
									...
									<input name="diskon_pesanan" class="form-control add-jumlah diskon_pesanan" placeholder="Besaran diskon (jika ada)" type="number" min="0" value="0">
								</div>
						...
						<form role="form" action="#" method="POST" id="editProductForm">
							...
							<div class="form-group mb-3">
								<div class="input-group input-group-merge input-group-alternative input-data-barang">
									...
									<input name="edit_diskon_pesanan" class="form-control add-jumlah edit_diskon_pesanan" placeholder="Diskon yang diberikan" type="number" min="0">
								</div>
...
<script>
	...
	function init() {
		...
		document.getElementById('tanggal_nota').value = now.toISOString().slice(0, 16);
	...
	$(document).ready(function() {
		...
		table.on('click', function() {
			...
			setTimeout(() => {
				...
				if (old_tdata == tdata) {
					...
					//menetapkan max min stok barang dipesan
					// $(".jumlah_pesanan").attr({
					// 	"max": tdata['brg_stok'],
					// 	"min": 1
					// });
					// $(".diskon_pesanan").attr({
					// 	"max": tdata['brg_hargajual'],
					// 	"min": 1
					// });
		...
		var table2 = $('#notaProductList').DataTable({
			...
			"columns": [{
				...
				{
					"mRender": function(data, type, row) {
						return '<div class="text-right"><a href="#/" data-idbrg="' + row.brg_id + '" data-idnota="' + row.jual_idnota + '" class="btn btn-warning btn-sm btnEdit mr-2 mb-2"><i class="fa fa-edit" style="color:white"></i></a><a href="#/" data-idbrg="' + row.brg_id + '"  data-idnota="' + row.jual_idnota + '" class="btn btn-danger btn-sm btnDelete mr-2 mb-2"><i class="fa fa-trash"></i></a></div>';
					}
				}
				//nama variabel yang digunakan adalah nama yang tertera di bagian "data", atau sama dengan nama kolom di tabel. Untuk ambil data per baris, tambah kata "row"
			],
		...
		$('#addNotaForm').submit(function(e) {
			...
			btn.html('<i class="fa fa-spin fa-spinner"></i> Menambah Nota...').attr('disabled', true);;
			$('.err').empty();
			$.ajax({
				method: 'POST',
				url: '<?php echo site_url('kasir/transactions/api?action=add_nota'); ?>',
				data: data,
				context: this,
				success: function(res) {
					if (res.code == 201) {
						...
						table2.ajax.url("<?php echo site_url('kasir/transactions/api?action=list_nota_products&nomor_nota='); ?>" + id_nota).load();

						//set nilai id nota hidden menjadi yang telah ditetapkan 
						$('.idnota_pesanan').val($('#nomor_nota').val());
		...
		$(document).on('click', '.cancelNotaBtn', function() {
			//data diidapat dari penggunaan atribut "name" pada html. nanti bisa dipanggil di modal
			var btn = $('.cancelNotaBtn');
			var id_nota = $('#nomor_nota').val();
			btn.html('<i class="fa fa-spin fa-spinner"></i> Membatalkan Nota...').attr('disabled', true);
			...
			$.ajax({
				...
				success: function(res) {
					if (res.data) {
						...
						//mengatur nilai max min di input
						// $(".edit_jumlah_pesanan").attr({
						// 	"max": d.brg_stok,
						// 	"min": 1
						// });
						// $(".edit_diskon_pesanan").attr({
						// 	"max": d.brg_hargajual,
						// 	"min": 1
						// });
