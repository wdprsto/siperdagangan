...
<section class="ftco-section ftco-Keranjang Belanja">
  <div class="container">
              ...
              <table class="table">
                ...
                <tbody>
                  <?php foreach ($carts as $item) : 
                    $currentItem =$this->product->get_product_by_id($item['id']);
                    ...
                    <tr class="text-center cart-<?php echo $item['rowid']; ?>">
                      ...
                      <td class="quantity">
                        <div class="input-group mb-3">
                          <input type="number" name="quantity[<?php echo $item['rowid']; ?>]" class="quantity form-control input-number" 
                          value="<?php 
  
                              echo $item['qty']; 

                          ?>" min="1" max="<?php echo  $currentItem->stock;?>">
                        </div>
                      </td>

                      <td class="total">Rp <?php 

                          echo format_rupiah($item['subtotal']); 
              ...
              <p class="d-flex">
                ...
                <span class="n-subtotal font-weight-bold">Rp 
                  <?php 
                      ...
                      echo format_rupiah($total_cart);

                  ?></span>
                <input type='hidden' name='subtotal' value='<?php echo $total_cart; ?>'>
              </p>
              <p class="d-flex">
                <span>Biaya pengiriman</span>
                <?php 
                  if ($total_cart >= get_settings('min_shop_to_free_shipping_cost')) : 
                ?>
    ...
    <?php else : ?>
      <div class="row">
        <div class="col-md-12 ftco-animate">
          <div class="alert alert-info" style='text-align: center;'>Tidak ada produk dalam keranjang.<br><?php echo anchor(base_url().'/#products', 'Jelajahi produk kami'); ?> dan mulailah berbelanja!</div>
        </div>
      </div>
    <?php endif; ?>
  </div>
  <input type='hidden' name='curStock' id='curStock' value='<?php echo $this->product->get_product_by_id?>'>
</section>
